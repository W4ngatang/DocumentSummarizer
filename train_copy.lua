...
function train(train_data, valid_data)
  ...
  sent_conv_proto = torch.zeros(valid_data.batch_l:max(), opt.max_sent_l, opt.num_kernels)
  sent_conv_grad_proto = torch.zeros(valid_data.batch_l:max(), opt.max_sent_l, opt.num_kernels) -- added by Jeffrey
  ...
  if opt.gpuid >= 0 then
    ...
    if opt.gpuid2 >= 0 then
      ...
      context_proto = context_proto:cuda()
      sent_conv_proto = sent_conv_proto:cuda()
      sent_conv_grad_proto = sent_conv_grad_proto:cuda()
      encoder_grad_proto = encoder_grad_proto:cuda()	 
    end
  end
  ...
  function train_batch(data, epoch)
    ...
    for i = 1, data:size() do
      ...
      local sentence_enc = sentence_enc_proto[{{1, batch_l}, {1, source_l}}]
      if opt.gpuid >= 0 then
        cutorch.setDevice(opt.gpuid)
      end
      ...
      for t = 1, source_l do
        ...
        sentence_enc[{{},t}]:copy(out[#out]) -- copy encoded sentence
        table.remove(out) -- remove encoded sentence
        rnn_state_enc[t] = out
      ...
      for t = 1, target_l do
        ...
        local decoder_input = {sentence_enc[t], context[t], table.unpack(rnn_state_dec[t-1])}
...
function main() 
  ...
  if opt.train_from:len() == 0 then
    ...
    sent_conv = make_sent_conv(valid_data, opt) -- added by Jeffrey
  ...
  layers = {encoder, decoder, sent_conv}
